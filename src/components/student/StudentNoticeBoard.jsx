...
export default function StudentNoticeBoard() {
  const { user } = useAuth();
  const studentId = user?.linkedEntityId || "stu-001";
  ...
  const loadNotices = useCallback(async () => {
    ...
    try {
      ...
      const readIds = new Set(
        userNotices
          .filter((n) => n.readReceipts?.some((r) => r.userId === studentId))
          .map((n) => n.id),
      );
  ...
  const handleMarkAsRead = async (noticeId) => {
    try {
      await markNoticeRead(noticeId, studentId);
  ...
  const {
    ...
    switch (activeFilter) {
      ...
      case "events":
        filtered = filtered.filter((n) =>
          [NOTICE_CATEGORIES.EVENT, NOTICE_CATEGORIES.HOLIDAY].includes(
            n.category,
          ),
        );
        break;
    ...
    return {
      academic: filtered.filter((n) =>
        [NOTICE_CATEGORIES.ACADEMIC].includes(n.category),
      ),
      exams: filtered.filter((n) =>
        [NOTICE_CATEGORIES.EXAMINATION].includes(n.category),
      ),
      assignments: filtered.filter((n) => n.requiresAction),
      timetable: filtered.filter((n) =>
        [NOTICE_CATEGORIES.TIMETABLE].includes(n.category),
      ),
      results: filtered.filter((n) =>
        [NOTICE_CATEGORIES.RESULTS].includes(n.category),
      ),
      events: filtered.filter((n) =>
        [NOTICE_CATEGORIES.EVENT, NOTICE_CATEGORIES.HOLIDAY].includes(
          n.category,
        ),
      ),
      discipline: filtered.filter((n) =>
        [NOTICE_CATEGORIES.DISCIPLINE].includes(n.category),
      ),
    };
      ...
      {academic.length > 0 && (
        <div className="bg-white rounded-2xl border border-gray-200 p-4">
          ...
          <NoticeSection
            ...
            onRead={handleMarkAsRead}
            readNoticeIds={readNoticeIds}
            emptyMessage="No academic updates"
          />
        </div>
      )}
      ...
      {timetable.length > 0 && (
        <div className="bg-white rounded-2xl border border-gray-200 p-4">
          ...
          <NoticeSection
            ...
            onRead={handleMarkAsRead}
            readNoticeIds={readNoticeIds}
            emptyMessage="No timetable changes"
          />
        </div>
      )}
      ...
      {events.length > 0 && (
        <div className="bg-white rounded-2xl border border-gray-200 p-4">
          ...
          <NoticeSection
            ...
            onRead={handleMarkAsRead}
            readNoticeIds={readNoticeIds}
            emptyMessage="No events or holidays"
          />
        </div>
      )}
      ...
      {discipline.length > 0 && (
        <div className="bg-white rounded-2xl border border-gray-200 p-4">
          ...
          <NoticeSection
            ...
            onRead={handleMarkAsRead}
            readNoticeIds={readNoticeIds}
            emptyMessage="No discipline notices"
          />
        </div>
      )}
